...
#include<math.h>
using namespace std;
//Программа, реализующая методы параллельного программирования при использовании рекурентных функций
//Выполняется вычисление интаграл от функции f() путем дробления подынтегральной области
//Выполняется сортировка массива путем разбиения его на отдельные части, поддающиеся сортировке
//Для сравнения, на экран выводится время работы линейной и параллельной версий программы
...
double parallel_integral(float a, float b, double f(double));			//Параллельное интегрирование
...
void parallel_qsort(int* A, int l, int r);												//Параллельная сортировка
...
#pragma omp single
		{
			cout << "parallel_integral(1, 10, f) = " << parallel_integral(1, 10, f) << endl; //Параллельное интегрирование
		}
	}
...
#pragma omp single
		{
			parallel_qsort(A, 0, 1'0000 - 1);																	//Параллельная сортировка
		}
	}
	cout << "Time parallel_quicksort:: " << omp_get_wtime() - t << endl;	//Время, затраченное на вычисления
...
double parallel_integral(float a, float b, double f(double))							//Параллельная рекурсивная фунцкия, вычисляющая интеграл функции
{																																	//f(double) на отрезке [a;b] через формулу трапеций
	double sum = integr(a, b, f);																		//Начальное приближение
	double sum1 = 0;
	double sum2 = 0;
...
#pragma omp task shared(sum1)
		sum1 = parallel_integral(a, (a + b) / 2, f);
#pragma omp task shared (sum2)
		sum2 = parallel_integral((a + b) / 2, b, f);
#pragma omp taskwait
		sum = sum1 + sum2;
	}
	return sum;
...
{																																	//чтобы слева от элемента A[r] стояли числа, не большие, а справа - не меньшие его
	...
	//индекс i возвращается вместе с отсортированным участком массива
		while (A[++i] < v);
		while (v < A[--j])
			if (j == l)
				break;
...
void parallel_qsort(int* A, int l, int r)																//Параллельная сортировка массива А
{	//До тех пор, пока крайние индексы l и r не станут удовлетворять условию l >= r,
	//функция будет сортировать данную часть массива с помощью функции partition
	...
	int i = partition(A, l, r);
#pragma omp task
	parallel_qsort(A, l, i - 1);
#pragma omp task
	parallel_qsort(A, i + 1, r);
}
