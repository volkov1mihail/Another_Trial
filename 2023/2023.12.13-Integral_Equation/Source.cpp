...
int main()
{
	...
	int n = 0;
	double a{ 0 }, b{ 0 };
	std::cout << "Численное решение интегрального уравнения" << std::endl;
	std::cout << "Введите границы промежутка [a,b]" << std::endl;
	std::cin >> a >> b;
	std::cout << "Введите число разбиений n" << std::endl;
	std::cin >> n;
	...
	//std::cout << std::endl << "Метод трапеций:" << std::endl;
	//result = solution(a, b, n, trapeze);
	//for (int i = 0; i < result.n_rows; ++i)
	//	std::cout << "u(x" << i << ") = " << result(i) << std::endl;

	std::cout << std::endl << "Метод трапеций, уравнение первого рода:" << std::endl;
	arma::vec result = solutionFirstKind(a, b, n, trapezeFirstKind);
	for (int i = 0; i < result.n_rows; ++i)
		std::cout << "u(x" << i << ") = " << result(i) << std::endl;


	//std::cout << "Сравнение в кратных точках, метод трапеций, уравнение первого рода:" << std::endl;
	...
	//std::cout << "Сравнение в кратных точках, метод средних прямоугольников:" << std::endl;
	...
	//std::cout << "Сравнение в кратных точках, метод трапеций:" << std::endl;
