...
int main()
{
	...
	cout << setprecision(13) << "„исленное решение «адачи  оши дл¤ обыкновенного дифференциального уравнени¤ первого пор¤дка" << endl;
	double x0, y0;
	cout << "¬ведите значение x0" << endl;
	cin >> x0;
	cout << "¬ведите значение y0" << endl;
	cin >> y0;
	c1 = exp(x0) * (y0 - x0 + 1);

	cout << "«адача  оши: y'(x) = -y(x) + x, y(" << x0 << ") = " << y0 << endl;
	cout << "“очное решение: y(x) = " << c1 << "*exp(-x)+x-1" << endl;
	double h;
	int N;
	cout << "¬ведите значение h" << endl;
	cin >> h;
	...
	cout << "ћетод разложени¤ в р¤д “ейлора" << endl;
