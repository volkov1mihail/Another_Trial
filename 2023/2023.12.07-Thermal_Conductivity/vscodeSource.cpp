...
int main(int args, char const *argv[])
{
  ...
  int window_width = -min(-(50 + 5 * N1), 500, 0, 0);
  int window_height = min((100 + 500 * u[N1][N2]), 500, 10000, 10000);
  ...
  std::cin >> DISTANCE;
  std::cout << "ALPHA_X = ?" << std::endl;
  std::cin >> ALPHA_X;
  std::cout << "BETA_Y = ?" << std::endl;
  std::cin >> BETA_Y;
  std::cout << "GAMMA_Z = ?" << std::endl;
  std::cin >> GAMMA_Z;
...
{
  double x1 = x;
  x = x * cos(BETA_Y) + z * sin(BETA_Y);
  z = -x1 * sin(BETA_Y) + z * cos(BETA_Y);
}

void rotate_z(double &x, double &y)
{
  double x1 = x;
  x = x * cos(GAMMA_Z) - y * sin(GAMMA_Z);
  y = x1 * sin(GAMMA_Z) + y * cos(GAMMA_Z);
}
...
void rotated_lines(int *result, double **array, double x0_max, double y0_max, double h, double tau)
{
  ...
  result[0] = 100 - min(bigArray[0][0], bigArray[1][0], bigArray[2][0], bigArray[3][0]);
  result[1] = 100 - min(bigArray[0][1], bigArray[1][1], bigArray[2][1], bigArray[3][1]);
